...
/* Deque is a sequence container thats is designed to allow very fast insertion and removal at both
ends of the container with a time complexity of O(1). However insertion and removal of other elements
in-between is in linear time O(n). Deque is similar to vector in some aspects like increasing the
size of the array dynamically but unlike vectors, the elements of deque are not in a contigious
memory space, instead every time a new array is neede to extend the deque, it is added as a seperate
fixed-size array this is cheaper than vector. The process of expansion and reduction happens
automatically.  */
