...
/* A map is an associative container meaning it assigns values to keys, thereby storing them
as key/value pairs. each key has to be unique, else the new value of the key will overide
the previous value. */
...
int main()
{
    ...
    std::map<std::string, int> spelling = {{"one", 1}, {"two", 2}, {"three", 3}};
    ...
    // A std::map can be thought of as a collection of std::pair's
    std::cout << std::endl;
    std::cout << "Looping around printing stuff : " << std::endl;
    std::cout << std::endl;
    std::cout << " printing books with regular range based for loop [] syntax (C++17) :\n";
