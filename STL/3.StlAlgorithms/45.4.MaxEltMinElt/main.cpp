...
int main()
{
    ...
    // std::vector<int> nums = {3,400,51,6,7,23,56,71};
    // int nums[] = {3,400,51,6,7,1123,56,71};
    std::list<int> nums = {3, 400, 51, 6, 7, 23, 56, 71};

    // max_elt and min_elt return an iterator to the found elt
    auto result = std::max_element(std::begin(nums), std::end(nums));
    std::cout << "max element is : " << *result << std::endl;

    result = std::min_element(std::begin(nums), std::end(nums));
    ...
    // Finding the closest
    result = std::min_element(std::begin(nums), std::end(nums), distance);
    std::cout << *result << " is closest to " << number_to_find << std::endl;

    // Finding the furthest
    result = std::max_element(std::begin(nums), std::end(nums), distance);
    std::cout << *result << " is furthest from  " << number_to_find << std::endl;

    std::cout << "--------------------------------" << std::endl;

    // Capturing min and max in a pair object with the auto syntax
    const auto [near, far] = std::minmax_element(std::begin(nums), std::end(nums), distance);
